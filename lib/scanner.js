...
/**
 * 買取スキャナー（hikaku-342505.firebaseapp.com）にログインし、
 * 商品を検索して各ショップの買取価格を取得する。
 *
...
export async function searchScannerPrices(query, { debug = false } = {}) {
  const email = process.env.KAITORI_SCANNER_EMAIL;
  const password = process.env.KAITORI_SCANNER_PASSWORD;

  if (!email || !password) {
    throw new ScannerError('買取スキャナーのログイン情報が設定されていません', { step: 'config' });
  }
  if (!query || !query.trim()) {
  ...
  try {
    ...
    try {
      ...
      throw new ScannerError('ログインページを開けませんでした', {
        step: 'goto_login',
        debugInfo: debug ? await captureDebug(page, 'goto_login') : null,
      });
    }

    try {
      const emailInput = page
        .locator('input[type="email"], input[name="email"], input[autocomplete="username"], input[autocomplete="email"]')
        .first();
      ...
      const loginButton = page.getByRole('button', { name: /ログイン|サインイン|login/i }).first();
      await loginButton.click();
    } catch {
      throw new ScannerError('ログインフォームの操作に失敗しました（画面構成が想定と異なる可能性）', {
        step: 'fill_login',
        debugInfo: debug ? await captureDebug(page, 'fill_login') : null,
      });
    }
    ...
    try {
      ...
      throw new ScannerError('ログインに失敗しました（IDまたはパスワードが正しいか確認してください）', {
        step: 'login_redirect',
        debugInfo: debug ? await captureDebug(page, 'login_redirect') : null,
      });
    }
